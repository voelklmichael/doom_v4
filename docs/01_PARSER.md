...
* **Statement-shaped macros** (`#define Z_ChangeTag(p,t) { if (...) I_Error(...); Z_ChangeTag2(p,t); };`, `#define DOOUTCODE(oc,mx,my) (oc)=0; if(...) ...;`): common for macros meant to be used as a statement, not a value -- a braced block, a flat sequence of statements with no enclosing braces, or even plain declarations (`#define SEQ_DEFINEBUF(len) unsigned char _seqbuf[len]; int _seqbuflen = len;`). Not a single expression (an `if` isn't an expression at all in C89), so when the single-expression parse fails, this step retries the same token stream as a *sequence of block items* -- exactly what Step 6c's `parse_compound_stmt` already parses inside a real `{ ... }`, just without requiring the enclosing braces, reusing the same declaration/statement grammar (`grammar::parse_block_items_from_tokens`). Still purely syntactic, still no argument substitution.
* **Target Representation**:
  ```rust
  pub enum MacroBody {
      Object(Expr),
      ...
      /// Not a single expression, but a real sequence of declarations
      /// and/or statements -- what you'd get if the body were pasted
      /// directly into a function body.
      Statements { params: Option<Vec<String>>, body: Vec<BlockItem> },
      /// Had replacement text, but it didn't reduce to a single expression or
      /// a parseable statement sequence -- kept for provenance/diagnostics,
      /// not a hard error (see `docs/KNOWN_LIMITATIONS.md`).
      Unparseable(String),
  }
  ```
* **Adjacent-literal substitution inside a body, not just in code**: a macro body can itself contain another macro sitting directly next to a string/char literal (e.g. `#define NETEND "...\n\n"PRESSKEY`, where `PRESSKEY` is its own plain literal-bodied macro) -- the same shape Step 4b already handles for code. So before lexing a body into tokens, it goes through Step 4b's `substitute_adjacent_literal_macros` too (using the same Step 4b literal-macro map for the file), turning that identifier into a literal token as well; Step 6c's existing adjacent-string-literal-concatenation then merges the two into one `Expr::StringLiteral`, same as it always does.
* **`__FILE__`/`__LINE__` are compiler-predefined, not `#define`d anywhere**: `z_zone.h`'s `Z_ChangeTag(p,t)` macro body includes `"Z_CT at "__FILE__":%i"` -- a string literal directly followed by `__FILE__` directly followed by another string literal, relying on the same adjacent-literal-concatenation idiom as the `NETEND`/`PRESSKEY` case above. But `__FILE__`/`__LINE__` come from the compiler's own preprocessor, so no `#define` for them exists anywhere in the corpus for `LiteralMacroResolver` to find. `macro_literals.rs` hardcodes both as a fallback (a placeholder string for `__FILE__`, a placeholder integer for `__LINE__` -- meaningless as *values* to a static pass with no real build behind it, but sufficient as literal *tokens* for substitution and parsing), applied unconditionally the same way `ImportResolver`'s system-typedef fallbacks are.
* **Validation Criteria**: Every `#define` visible to (i.e. transitively reachable from) each of the 62 `.c` translation units gets parsed into a `MacroBody`; corpus scan reports how many resolve to `Object`/`Function`/`Empty`/`Statements` vs. fall back to `Unparseable`, matching Step 4b's "measure actual scope before deciding it needs more" methodology. Over the full corpus: 19136 object-like and 960 function-like macro occurrences parse to a single expression, 4459 are genuinely empty flag macros, 39 parse as a statement/declaration sequence, and 1421 have bodies that are neither (a bare type name, a lone storage-class keyword, ioctl-style macros whose "argument" is a type keyword, ...) and stay `Unparseable`.
